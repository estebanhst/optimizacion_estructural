...
ax(6) = nexttile(6); hold off;
imagesc(s1); axis equal tight; colormap(ax(6), jet);colorbar;
hold on; 
quiver(cos(ang), sin(ang), 1, 'k','ShowArrowHead','off');
title('\sigma_{I}')
...
ax(9) = nexttile(9); hold off;
imagesc(s2); axis equal tight; colormap(ax(9), jet);colorbar;
hold on; 
quiver(cos(ang+pi/2), sin(ang+pi/2), 1, 'k','ShowArrowHead','off');
title('\sigma_{II}')
...
ax(12) = nexttile(12); hold off;
imagesc(tmax); axis equal tight; colormap(ax(12), jet);colorbar;
hold on; 
quiver(cos(ang+pi/4), sin(ang+pi/4), 1, 'k','ShowArrowHead','off');
title('\tau_{max}')
end
